<template>
  <section class="sale new">
    <div class="sale__container">
      <div class="sale__block-top">
        <div class="sale__block-left">
          <h1 class="sale__title">Новинки</h1>

        </div>
        <div class="sale__block-right">
          <div ref="nextBtn" class="sale__swiper-button-next new-next"></div>
          <div ref="prevBtn" class="sale__swiper-button-prev new-prev"></div>
        </div>
      </div>
      <div class="sale__block-middle">
        <div class="sale__swiper swiper mynewCards">
          <div class="sale__swiper-wrapper swiper-wrapper">
            <div v-for="(card, index) in cards" :key="index" class="sale__swiper-slide swiper-slide">
              <div class="sale__card new-card">
                <img :src="card.image" alt="" class="sale__card-img" @click="openModal(card)" />
                <span class="new-name">Новинка</span>
                <!-- <button class="sale__btn-like btn" :class="{ like: isInWhishList(card.id) }"
                  @click="toggleLike(card)"></button> -->
                  ...
                  <div class="sale__card-bottom">
                    <div class="sale__card-bottom-left">
                      <div class="sale__card-redprice">
                        <span class="sale__card-red new-price">{{ card.price }}₽</span>
                        <span class="sale__card-units">{{ card.units }}</span>
                      </div>
                    </div>
                    <div class="sale__card-bottom-rigth">
...
export default {
  ...
  computed: {
    ...mapGetters(['isInWhishList']), // Подключаем геттер для проверки
  },
  props: {
    product: String
  },
  methods: {
    ...
    async fetchProducts() {
      try {
        ...
        this.cards = response.data.filter(product => Boolean(product.is_new));

        console.log('Загруженные товары:', this.cards);
    ...
    initSwiper() {
      ...
      this.swiperInstance = new Swiper('.mynewCards', {
